        ...
        <ul className={classes.list}>
          {list.map((s, i) => (
            <li key={s}>
              ...
              {children && children[i] && (
                <ul className={classes.list}>
                  {children[i].map((s2: string, j: number) => (
                    <li key={s2}>
                      <Typography variant="body2">{s2}</Typography>
                      <ul className={classes.list}>
                        {children[`${i}_${j}`] &&
                          children[`${i}_${j}`].map((s3: string) => (
                            <li key={s3}>
  ...
  return (
    <Container maxWidth="xl" className={classes.root}>
      ...
      <Typography variant="subtitle1" gutterBottom paragraph>
        管理人は，本ウェブサイト上で提供するサービス（以下,「本サービス」といいます。）における，ユーザーの個人情報の取扱いについて，以下のとおりプライバシーポリシー（以下，「本ポリシー」といいます。）を定めます。
      </Typography>
      {simpleList(
        "第1条（個人情報）",
        [],
      ...
      {simpleList(
        ...
        [],
        {},
        "管理人は，ユーザーが利用登録をする際にメールアドレスなどの個人情報をお尋ねすることがあります。"
      )}
      {simpleList(
        "第3条（個人情報を収集・利用する目的）",
        [
